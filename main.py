...
def global_css(project_root: str, plan_path: str) -> None:
    ...
    # 1. Defensive Layer: Safely unwrap if the whole plan is wrapped in a list
    while isinstance(plan, list):
        if not plan:
            print("[Warning] plan.json is an empty list. Skipping global CSS injection.")
            return
        plan = plan[0]
    ...
    if isinstance(styling, dict):
        if "global_css" in styling:
            ...
            # Fallback instead of throwing an assertion crash
            print(f"[Warning] Unexpected styling sub-format dictionary. Stringifying fallback.")
            styling = str(styling)
            
    # 5. Ensure paths exist and assign code
...
def run_executor(plan_file: str, provider: str, max_workers: int, memory_str: str = "") -> None:
    ...
    # ── DEFENSIVE UNWRAPPING LAYER ──────────────────────────────────────────
    # Safely extract the inner dictionary if the LLM wrapped the entire plan in a list
    while isinstance(plan, list):
        if not plan:
            print(f"[Executor Error] Bypassing execution! {plan_file} parsed as an empty list.")
            return
        plan = plan[0]

    if not isinstance(plan, dict):
        print(f"[Executor Error] Expected plan to be a dictionary, but received: {type(plan)}")
        return
    # ──────────────────────────────────────────────────────────────────────────
...
def run_assigner(project_root: str = "spiderman", step_dir: str = STEP_PATH, max_workers: int = None):
    ...
    print(f"\n[Assigner] Processing {total_tasks} generation assets sequentially...\n")
    
    # Sequential loop featuring clean terminal process feedback
    for i in tqdm(range(total_tasks), desc="Building Project Components", unit="step"):
        try:
            run_task(i, total_tasks, project_root)
        except Exception as e:
            print(f"\n[Assigner Error] Execution broke unexpectedly at step index {i}: {e}")
